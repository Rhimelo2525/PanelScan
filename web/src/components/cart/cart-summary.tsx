...
export function CartSummary({
  ...
  const subtotal = selectedItems.reduce<number | null>((total, item) => {
    ...
    <aside className="rounded-xl border border-border bg-secondary/45 p-6 lg:sticky lg:top-28" aria-labelledby="cart-summary-title">
      ...
      <dl className="mt-6 space-y-4 border-b border-border pb-5 text-sm">
        <div className="flex justify-between gap-4">
          <dt className="text-muted-foreground">Item quantity</dt>
          <dd className="font-medium tabular-nums">{selectedItemCount}</dd>
        </div>
        <div className="flex items-end justify-between gap-4">
          <dt className="text-muted-foreground">Subtotal</dt>
          <dd className="text-xl font-semibold tabular-nums">{subtotal === null ? "Unavailable" : formatMinorUnits(subtotal)}</dd>
        </div>
      </dl>

      {!hasSelected ? (
        <div className="mt-5 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-xs leading-5 text-amber-800 dark:text-amber-300">
          <AlertCircle className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
          Please select at least one product to continue.
        </div>
      ) : canCheckout ? (
        <div className="mt-5 border-l-2 border-primary bg-background/70 px-4 py-3">
          <p className="text-sm font-semibold">Ready for checkout</p>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">You'll add delivery information and review the order before it is created.</p>
        </div>
      ) : (
        <div className="mt-5 flex items-start gap-2 rounded-lg border border-destructive/25 bg-destructive/5 px-4 py-3 text-xs leading-5 text-destructive">
          <AlertTriangle className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
          {checkoutWarning ?? cartError ?? "This cart cannot proceed to checkout."}
        </div>
      )}

...
**GET `/api/auth/google`**
- Initiates Google OAuth 2.0 customer login flow.
- Generates a cryptographically random `state` (stored in a secure HttpOnly `panelscan_oauth_state` cookie for CSRF protection).
- Redirects to Google's OAuth consent screen requesting only minimal scopes (`openid email profile`).
- Optional query parameter: `?returnTo=/path` (validated against open-redirect attacks).

**GET `/api/auth/google/callback`**
- Receives authorization callback from Google (`?code=...&state=...`).
- Verifies CSRF state against the cookie.
- Exchanges code for tokens with Google and verifies ID token signature, audience, and expiration.
- Resolves customer account in CockroachDB (creates new `CUSTOMER` or links to verified email account).
- Generates a short-lived single-use ticket and redirects to `${FRONTEND_URL}/login?ticket=<ticket>&from=<returnTo>`. Access tokens and refresh tokens are **never** exposed in redirect URLs.

**POST `/api/auth/google/exchange`**
Body: `{ "ticket": "<single-use-ticket>" }`
- Redeems the temporary exchange ticket for application session tokens.
- The ticket is immediately destroyed upon first use (replay attack protection).
- Returns: `{ "user": {...}, "token": "<JWT access token>", "refreshToken": "<opaque refresh token>" }`.

**POST `/api/auth/google`**
Body: `{ "credential": "<Google ID token>" }` or `{ "code": "<Google auth code>" }`
- Direct server-side verification endpoint for Google credentials.
- Cryptographically verifies Google signature, issuer, audience (`GOOGLE_CLIENT_ID`), expiration, and `email_verified: true`.
- Resolves or creates `CUSTOMER` account, issues session JWT and refresh token.

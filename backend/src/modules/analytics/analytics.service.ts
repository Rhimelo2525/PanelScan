...
export class AnalyticsService {
  ...
  private async countLowStock(): Promise<number> {
    const now = Date.now();
    if (env.NODE_ENV !== 'test' && this.lowStockCache && this.lowStockCache.expiresAt > now) {
  ...
  private async sumInventoryValue(): Promise<number> {
    const now = Date.now();
    if (env.NODE_ENV !== 'test' && this.inventoryValueCache && this.inventoryValueCache.expiresAt > now) {
